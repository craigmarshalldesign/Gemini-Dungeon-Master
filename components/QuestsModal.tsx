    ...
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        <div className="flex justify-between items-center border-b-2 border-gray-600 pb-2 mb-4">
          <h2 className="text-2xl">Quest Log</h2>
          <button onClick={onClose} className="text-2xl font-bold">&times;</button>
        </div>
        
        <div className="mb-6">
            <h3 className="text-xl text-yellow-300 mb-2 pb-1">Main Quest: {worldName}</h3>
            <p className="text-sm text-gray-300">{mainStory}</p>
        </div>

        <div>
           <h3 className="text-xl text-yellow-300 mb-2 pb-1">Local Quests</h3>
                    ...
                    {localQuests.map(quest => (
                        <li key={quest.id}>
                            <div className="flex justify-between items-baseline">
                                ...
                                <span className={`text-xs px-2 py-0.5 rounded ${quest.status === QuestStatus.COMPLETED ? 'bg-green-700' : 'bg-yellow-700'}`}>
                                    {quest.status}
                                </span>
                            </div>
                            <p className="text-sm text-gray-300 ml-4 mt-1">{quest.description}</p>
                        </li>
                    ))}
                </ul>
           ) : (
            <p className="text-gray-500">No active local quests.</p>
           )}
        </div>
      </div>
    </div>
